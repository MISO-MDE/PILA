...
export class IntermediaryFormComponent implements OnInit {
  ...
  public saveSuperEntity(): void {
    ...
    this.intermediaryApiService.saveSuperEntity(this.superEntity).subscribe((response: any) => {
      if (response) {
        ...
        this.userApiService.saveUserFirebase(this.userAdmin.email, this.userAdmin.password).then((result: any) => {
          ...
          this.userApiService.saveUser(this.userAdmin).subscribe((status: boolean) => {
            if (status) {
